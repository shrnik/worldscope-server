...
export const AGENT_TOOLS = [
  ...
  { name: 'adjust_params',
    description: 'Nudge pose parameters by small deltas and see the result: applies the deltas (clamped per call: |d_yaw|<=5, |d_pitch|<=3, |d_roll|<=2 degrees, |d_f_pct|<=10 percent, |d_height_m|<=3 m, |d_east_m|/|d_north_m|<=5 m), then returns the fresh annotated render plus the new parameters and RMS. Reading the render: EVERYTHING shifted uniformly left/right regardless of distance => yaw; NEARBY geometry misaligned much more than distant geometry (parallax) => camera position is off, use d_east_m/d_north_m to slide it; projected verticals leaning vs real ones => d_roll; ground geometry sitting too high/low => d_pitch and/or d_height_m; everything uniformly too large/small => d_f_pct. The given camera GPS is rarely exact — translating it is often the real fix. Make ONE small adjustment per call and compare renders. RMS may be n/a when no pairs exist — judge visually.',
    input_schema: { type: 'object', properties: {
      ...
      d_east_m: { type: 'number', description: 'Camera position shift east in meters (west if negative, clamped to ±5)' },
      d_north_m: { type: 'number', description: 'Camera position shift north in meters (south if negative, clamped to ±5)' },
