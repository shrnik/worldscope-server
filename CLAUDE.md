...
- The embedding checkpoint (`EMBED_MODEL`, default `google/tipsv2-b14`, loaded via `AutoModel` with `trust_remote_code=True`) must be the same in the server, `jobs/embed_job.py`, and `space/app.py` — image and text embeddings must share one space. Changing it changes the embedding dimension, so a full embeddings refresh is required before the Space/server can search again.
